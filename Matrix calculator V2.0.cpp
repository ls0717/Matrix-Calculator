...
{
	printf("-------------------------------------------------------------------------------\n\n");
	printf("-----------------------Welcome to Matrix calculator V2.0-----------------------\n\n");
...
int setting()
{
	...
	std::cout<<"3.输出中间步骤"<<std::endl;
	std::cout<<"4.不输出中间步骤"<<std::endl;
	std::cout<<"0.回到上一级"<<std::endl;
	int Flag=getint();
	if(!Flag) return 0;
	if (Flag<0||Flag>5)
...
int about()
{
	...
	std::cout<<"-------------本次加入了矩阵求逆和输出过程功能--------------"<<std::endl;
	std::cout<<"-------------后续会加入LU分解,导出到表格等功能--------------"<<std::endl;
...
int function_select()
{
	...
	std::cout<<"4.矩阵求逆"<<std::endl;
	std::cout<<"8.功能更新及源代码"<<std::endl;
